        ...
        output+=`
          <div class="col-md-3">
            <div class="well text-center">
              ...
              <h5>${movie.title}</h5>
              <a onclick="movieSelected('${movie.id}')" class="btn btn-primary" href="/movi">Movie Details</a>
            </div>
          </div>
        `;
      });
...
{
  let movieId = sessionStorage.getItem('movieId');
  axios.get("https://api.themoviedb.org/3/movie/" + movieId + "/videos?api_key=98325a9d3ed3ec225e41ccc4d360c817")
      .then(function (response) {
      let mtrailer = response.data;
      let out= `
      <a href="https://youtu.be/${mtrailer.results[0].key}" target="_blank" class="btn btn-primary">VIEW TRAILER</a>
      `;
      $('#movie').html(out);
    })
    .catch(function (error) {
